...
class EnemyObject(BaseObject):
    ...
    def events(self):
        if self.hp < 1:
            ...
            #Àû Ã³Ä¡ ½Ã ¾ÆÀÌÅÛ »ý¼º
            #È®·ü Ãß°¡ (Àû Ã³Ä¡ ½Ã 1/3È®·ü·Î ½Ã°£¾ÆÀÌÅÛ or 1/6È®·ü·Î µ¥¹ÌÁö¾ÆÀÌÅÛ or 1/6È®·ü·Î ¸ñ¼û¾ÆÀÌÅÛ µå¶ø, 1/3È®·ü·Î ¾ÆÀÌÅÛ ¹Ì»ý¼º) (È®Á¤Àº ¾Æ´Ô)
...
class EffectObject(BaseObject):
    ...
    def events(self):
        ...
        if self.types == 'player_shot':       # ÇÃ·¹ÀÌ¾î °ø°ÝÀÏ °æ¿ì
            self.animation('loop')
...
def createMapData():
    ...
    ground_mode_stack = 1       # Å« Å©±â ½ºÅÃ (Å« Å©±â 1~4°³ »ý¼º ÈÄ ÀÛÀº Å©±â 1°³ »ý¼º)
    ground_mode_stackMax = random.randrange(2, 6)

    for i in range(TILE_MAPSIZE[0] - 1):
        if ground_interval > 0:         # ¹Ù´Ú °£°Ý ¶ç¿ì±â
            floor_map[i] = -1
            ground_interval -= 1
        else:
            ...
            else:               # ¹Ù´Ú ¿Ï¼º½Ã ´ÙÀ½ ¹Ù´Ú Å©±â ¹× °£°Ý Å©±â Ã³¸®
...
def createMapImage(tileSpr, structSpr):
    ...
    for col in range(TILE_MAPSIZE[0] - 1):
        if floor_map[col] == -1:     # ºñ¾úÀ» °æ¿ì
            ...
            for backtile in range(5 + back_height - floor_map[col]):        # Å¸ÀÏ µÞºÎºÐ Ã¤¿ì±â
                if backtile < 5:
            ...
            image.blit(tileSpr.spr[spr_index], (col * TILE_SIZE, floor_map[col] * TILE_SIZE))   # Å¸ÀÏ ¾ÕºÎºÐ Ã¤¿ì±â
