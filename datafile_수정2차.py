...
class BaseObject:
    ...
    def Dash(self):
        ...
        if self.dash_current_use > 0:
            ...
            if self.facing_left:                        # 왼쪽을 보고있을 때 대시
               self.vx = -1.5 * self.speed
               self.ax = (self.speed * 0.5)
               self.status = -3
               self.dash_current_use -= 1

            if self.facing_right:                        # 오른쪽을 보고있을 때 대시
               self.vx = 1.5 * self.speed
               self.ax = (self.speed * 0.5)
               self.status = -3
               self.dash_current_use -= 1
        ...
        threading.Timer(5, self.start())       # 0에서 15초 사이로 랜덤하게 돌진 실행
...
class BossObject(BaseObject):
    def __init__(self, spr, coord, kinds, game, types):
        ...
        self.Boss_frame = 0                    # 플레이어 애니메이션 프레임
        self.Boss_frameSpeed = 1               # 플레이어 애니메이션 속도(낮을 수록 빠름. max 1)
        self.Boss_frameTimer = 0
        self.Boss_flip = False                 # 플레이어 이미지 반전 여부 (False: RIGHT)
